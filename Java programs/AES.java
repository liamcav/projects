/*
Liam Caveney 
SENG 360, October 2016

Program encrypts or decrypts using AES
$$>java AES option keyFile inputFile
*/

import java.io.*;
import java.util.*;
import java.nio.file.Files;
import java.nio.file.Paths;
import java.nio.file.Path;



public class AES{
	private boolean encrypt;
	private int[] key;
	private ArrayList<String> messageList;
	
	public AES(boolean e, int[] k, ArrayList<String> in){
		encrypt=e;
		key=k;
		messageList=in;
	}	
	
	public int[] encrypt(){
		String[] message = messageList.toArray(new String[0]);
		int[][] intlines=new int[message.length][16];
		int i;
		for (i=0; i<message.length; i++){
			intlines[i]=encryptLine(message[i]);
		}
		int[] out=new int[(intlines.length)*(intlines[0].length)];
		int count=0;
		for (int k=0; k<intlines.length; k++){
			for (int j=0; j<16; j++){
				out[count]=intlines[k][j];
				count++;
			}
		}
		return out;	
	}

	public int[] decrypt(){
		String[] message = messageList.toArray(new String[0]);
		int[][] intlines=new int[message.length][16];
		int i;
		for (i=0; i<message.length; i++){
			intlines[i]=decryptLine(message[i]);
		}
		int[] out=new int[(intlines.length)*(intlines[0].length)];
		int count=0;
		for (int k=0; k<intlines.length; k++){
			for (int j=0; j<16; j++){
				out[count]=intlines[k][j];
				count++;
			}
		}
		return out;	
		
	}
	
	private int[] encryptLine(String line){
		int[] state=new int[16];
		
		if (line.length()!=32){ 
			System.out.println("Error, line not of correct length");
			System.exit(0);
		}	
		int count=0;
		for (int i=0; i<16; i++){
			state[i]=Integer.decode("0x"+line.charAt(count++)+line.charAt(count++));
		}		
	
		int[] keysched=KeyExpansion(key);
	
		//Initial Round
		state=AddRoundKey(state, keysched, 0);

		//Rounds
		int i;
		for (i=1; i<13; i++){			
			state=SubBytes(state);		
			state=ShiftRows(state);
			state=MixColumns(state);		
			state=AddRoundKey(state, keysched, i);					
		}
		//Final Round
		state=SubBytes(state);
		state=ShiftRows(state);
		state=AddRoundKey(state, keysched, 13);
		
		return state;	
	}		
	
	private int[] decryptLine(String line){
		int[] state=new int[16];
		if (line.length()!=32){ 
			System.out.println("Error, line not of correct length");
			System.exit(0);
		}	
		int count=0;
		for (int i=0; i<16; i++){
			state[i]=Integer.decode("0x"+line.charAt(count++)+line.charAt(count++));
		}	
		int[] keysched=KeyExpansion(key);
		//Initial Round
		state=AddRoundKey(state, keysched, 13);
		state=InvShiftRows(state);	
		state=InvSubBytes(state);		
		//Rounds
		for (int i=12; i>0; i--){
			state=AddRoundKey(state, keysched, i);	
			state=InvMixColumns(state);		
			state=InvShiftRows(state);
			state=InvSubBytes(state);
		}
		//Final Round
		state=AddRoundKey(state, keysched, 0);

		return state;
	}
	
	private int[] KeyExpansion(int[] key){
		
		int[] s_box=new int[]{
			
		};
		
		int rcon[] = {
			
			};
		
		int[] expkey =new int[240];
		int cur=32;
		int count=1;
		for (int i=0; i<32; i++){
			expkey[i]=key[i];
		}
		
		while(cur<240){
			//for (int i=0; i<expkey.length; i++) System.out.print(expkey[i]+ " ");
			//System.out.println();
			int[] t=new int[4];
			for(int i=0; i<4; i++){
				t[i]=expkey[i+cur-4];
				//System.out.println(t[i]);
			}
			
			if(cur%32==0){	//schedule core
				t=RotWord(t);
				//for(int i=0; i<t.length; i++) System.out.println("rot " +t[i]);
				for (int i=0; i<4; i++){
					t[i]=s_box[t[i]];
				}
				//for (int i=0; i<t.length; i++) System.out.println("t[" +i +"]: " +t[i]);
				t[0]^=rcon[count];
				count++;
			}
			if(cur%32==16){
				for(int i=0; i<4; i++){
					t[i]=s_box[t[i]];
				}
			}
			for(int i=0; i<4; i++){
				expkey[cur]=(expkey[cur-32]^t[i]);
				//System.out.println(cur + " " + expkey[cur]);
				cur++;
			}
		}
		return expkey;
	}		

	private int[] RotWord(int[] in){
		int[] out=new int[4];
		for (int i=0; i<3; i++){
			out[i]=in[(i+1)];
		}
		out[3]=in[1];
		return out;
	}		
	
	private int[] SubWord(int[] in){
		int[] s_box=new int[]{
			
		};
		
		int[] temp = new int[in.length];
		for (int i=0; i<temp.length; i++){
			temp[i]=(s_box[in[i] & 0x000000ff] & 0xff);
		}
		return temp;
	}		
	
	private int[] SubBytes(int[] state){
		int[] s_box=new int[]{
			
		};
		
		for (int i=0; i<16; i++){
			state[i]=s_box[state[i]];
		}
		return state;	
	}
	
	private int[] InvSubBytes(int[] state){
		int[] inv_s_box=new int[]{
			
		};
		
		for (int i=0; i<16; i++){
			state[i]=inv_s_box[state[i]];
		}
		return state;	
	}
	
	private int[] ShiftRows(int[] state){
		int[] temp=new int[16];
		temp[0]=state[0];
		temp[1]=state[5];
		temp[2]=state[10];
		temp[3]=state[15];
		temp[4]=state[4];
		temp[5]=state[9];
		temp[6]=state[14];
		temp[7]=state[3];
		temp[8]=state[8];
		temp[9]=state[13];
		temp[10]=state[2];
		temp[11]=state[7];
		temp[12]=state[12];
		temp[13]=state[1];
		temp[14]=state[6];
		temp[15]=state[11];
		return temp;
	}	
	
	private int[] InvShiftRows(int[] state){
		int[] temp=new int[16];
		temp[0]=state[0];
		temp[5]=state[1];
		temp[10]=state[2];
		temp[15]=state[3];
		temp[4]=state[4];
		temp[9]=state[5];
		temp[14]=state[6];
		temp[3]=state[7];
		temp[8]=state[8];
		temp[13]=state[9];
		temp[2]=state[10];
		temp[7]=state[11];
		temp[12]=state[12];
		temp[1]=state[13];
		temp[6]=state[14];
		temp[11]=state[15];
		return temp;
	}	

	private int[] AddRoundKey(int[] state, int[] keysched, int round){
		int[] roundkey=new int[16];
		round++;
		for (int i=0; i<16; i++){
			roundkey[i]=keysched[16*round+i];
		}	
		
		for (int i=0; i<16; i++){
			state[i]^=roundkey[i]; 
		}
		return state;	
	}	
	
	private int[] MixColumns(int[] state){
		int[] mul2=new int[]{
			
		};
		
		int[] mul3=new int[]{
			
		};
		
		int[] temp=new int[16];
		
		temp[0] = (mul2[state[0]] ^ mul3[state[1]] ^ (int)state[2] ^ (int)state[3]);
		temp[1] = (state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3]);
		temp[2] = (state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]]);
		temp[3] = (mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]]);

		temp[4] = (mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7]);
		temp[5] = (state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7]);
		temp[6] = (state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]]);
		temp[7] = (mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]]);

		temp[8] = (mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11]);
		temp[9] = (state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11]);
		temp[10] = (state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]]);
		temp[11] = (mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]]);

		temp[12] = (mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15]);
		temp[13] = (state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15]);
		temp[14] = (state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]]);
		temp[15] = (mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]]);

		return temp;	
	}	
	
	private int[] InvMixColumns(int[] state){
		int[] mul9=new int[]{
			
		};
		
		int[] mul11=new int[]{
			
		};
		
		int[] mul13=new int[]{
			
		};
		
		int[] mul14=new int[]{
			
		};
		
		
		int[] temp=new int[16];
	
		temp[0] = (mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]]^ mul9[state[3]]);
		temp[1] = (mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]]);
		temp[2] = (mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]]);
		temp[3] = (mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]]);

		temp[4] = (mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]]^ mul9[state[7]]);
		temp[5] = (mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]]);
		temp[6] = (mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]]);
		temp[7] = (mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]]);
	
		temp[8] = (mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]]^ mul9[state[11]]);
		temp[9] = (mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]]);
		temp[10] = (mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]]);
		temp[11] = (mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]]);
		
		temp[12] = (mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]]^ mul9[state[15]]);
		temp[13] = (mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]]);
		temp[14] = (mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]]);
		temp[15] = (mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]]);

		return temp;	
	}	
	
	public static void main(String[] args)throws IOException{
		Scanner sk=new Scanner(new File(args[1]));
		int[] keyi=new int[32];
		String keystring=sk.next();
		int count=0;
		for (int i=0; i<32; i++){
			keyi[i]=Integer.decode("0x"+keystring.charAt(count++)+keystring.charAt(count++));
		}
		sk.close();
		
		Scanner s = new Scanner(new File(args[2]));
		ArrayList<String> inputlist = new ArrayList<String>();
		while (s.hasNext()){
			inputlist.add(s.next());
		}
		s.close();
		
		if (args[0].charAt(0)=='e'){
			AES message=new AES(true, keyi, inputlist);
			int[] output=message.encrypt();
			
			PrintWriter outfile = new PrintWriter(args[2]+".enc");
			for(int i=0; i<output.length; i++){
				String str=Integer.toHexString(output[i]);
				if(str.length()<2){
					str="0"+str;
				}	
				outfile.print(str.toUpperCase());
				if(i%16==15){
					outfile.println();
				}
			}
			outfile.close(); 
	
		}
		else if (args[0].charAt(0)=='d'){
			AES message=new AES(false, keyi, inputlist);
			int[] output=message.decrypt();
			
			PrintWriter outfile = new PrintWriter(args[2]+".dec");
			for(int i=0; i<output.length; i++){
				String str=Integer.toHexString(output[i]);
				if(str.length()<2){
					str="0"+str;
				}	
				outfile.print(str.toUpperCase());
				if(i%16==15){
					outfile.println();
				}	
			}
			outfile.close(); 
			
		}	
		else { 
			System.out.println("Error: args[0] must be 'e' or 'd'");
			System.exit(0);
		}
	}

}